...
| $H_a$ Camera Head Node | Co-located ESP32-S3 + OV5640 5MP camera + MAX3490 RS-422 transceiver in IP67 camera head | Integrated IP67 camera head with ESP32-S3 node | selected | Mounts directly over $H_a$ staff gauge. Runs C++ vision algorithm and 5-stage DSP locally; transmits computed level ($0.08\text{ ft}$) and JPEGs over RS-422 bus. |
| $H_b$ Submergence Camera Head Node | Second co-located ESP32-S3 camera node in IP67 head | Integrated IP67 camera head with ESP32-S3 node | selected | Mounts over downstream $H_b$ throat gauge for Parshall submergence detection; transmits data over RS-422 bus. |
