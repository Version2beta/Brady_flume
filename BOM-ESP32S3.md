...
| Camera Cross-Arm Support Bar | Rigid 3.5-foot Aluminum / Stainless Cross-Arm Support Bar Assembly | Heavy-duty Unistrut / Aluminum Arm | selected | Mounts $3.5\text{ feet}$ ($42\text{ in}$) out from flume wall. Holds $H_a$ camera box, $H_b$ camera box, and single off-axis IR spotlight at $D = 3.5\text{ ft}$. |
...
| $H_a$ Camera Head Assembly | Primary upstream staff gauge camera head node | Compact IP67 clear-lid camera node | selected | Mounts directly over $H_a$ staff gauge. Transmits computed level ($0.08\text{ ft}$) and JPEGs over RS-422 bus. |
| $H_b$ Submergence Camera Assembly | Secondary throat staff gauge camera head node | Compact IP67 clear-lid camera node | selected | Identical camera head node mounted over downstream $H_b$ throat gauge for Parshall submergence detection over RS-422 bus. |
...
| Shared Night IR Illuminator | Single Standalone IP67 850nm IR Illuminator Spotlight (Off-Axis Mounted) | 12V/24V 850nm IP67 IR Spotlight (Qty 1) | selected | **CRITICAL OPTICS NOTE:** A single IP67 850nm IR Spotlight with a $60^\circ$ flood beam mounted 2–3 feet off-axis to the side of the flume illuminates **both** $H_a$ and $H_b$ staff gauges simultaneously. Off-axis angle prevents retroreflective white gauge glare. Gated via GPIO MOSFET relay for 2-second bursts. |
